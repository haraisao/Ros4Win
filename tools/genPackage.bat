...
@rem if "%BUILD_DIR%" == ""   @set BUILD_DIR=R:/_build/%PKG_NAME%
@if "%BUILD_TYPE%" == ""  @set BUILD_TYPE=Release

@set ERRORLEVEL=0
@echo ===== Build and install ( %PKG_NAME% ) =====
@echo;
cmake -G "Visual Studio 15 2017 Win64"  -DCATKIN_ENABLE_TESTING=0 -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DCMAKE_INSTALL_PREFIX=%INSTALL_DIR% -B %BUILD_DIR% -S  %PKG_NAME%
