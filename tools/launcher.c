...
char *find_exe(char *exename, char *script) {
    ...
    /* use environment variabel 'PYTHON_EXE' */
    char *python_script=getenv("PYTHON_EXE");
    if(python_script){
        return loadable_exe(python_script);  
    }
...
int run(int argc, char **argv, int is_gui) {
    ...
    *ptr-- = '\0';
    ...
    if (strncmp(python, "#!", 2) || !strncmp(python, "#!/usr/bin/env python", 21)
         || !strncmp(python, "#! /usr/bin/env python", 22) ) {
